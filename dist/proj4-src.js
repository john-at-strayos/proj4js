(function (global, factory) {
    ...
    var globals = function(defs) {
      ...
      defs('EPSG:3857', "+title=WGS 84 / Pseudo-Mercator +proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +no_defs");
      for(var i=0;i<60;++i){
    	  defs('EPSG:'+(32600+i),'+proj=utm +zone='+i+' +north +datum=WGS84 +units=meter +no_defs');
    	  defs('EPSG:'+(32700+i),'+proj=utm +zone=${i} +south +datum=WGS84 +units=meter +no_defs');
      }
    ...
    */
    function geocentricToWgs84(p, datum_type, datum_params) {
      ...
      if (datum_type === PJD_3PARAM) {
        ...
        p.x=M_BF * (x - Rz_BF * y + Ry_BF * z) + Dx_BF;
        p.y=M_BF * (Rz_BF * x + y - Rx_BF * z) + Dy_BF;
        p.z=M_BF * (-Ry_BF * x + Rx_BF * y + z) + Dz_BF;
        return p;
      }
    ...
    function transform(source, dest, p) {
    	var wgs84;
    	if (Array.isArray(p)) {
    		p = toPoint(p);
    	}else{
